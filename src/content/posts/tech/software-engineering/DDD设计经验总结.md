...
领域层（Domain Layer）采用 Kotlin 以强化约束：领域层包含长任务状态流转、知识库关联等核心业务规则。利用 Kotlin 严格的编译器级别约束（如空安全机制、默认不可变集合、数据类及密封类），可以更严谨地实现 DDD 中值对象的不可变性以及状态机的穷举校验。这部分核心模型由架构设计者统一使用 Kotlin 构建。
...
在原本设计中，需要提取树结构，然后在业务层将树和源文件绑定，最后再持久化，导致太多细节外漏，所以在后续反思中重构了此处代码：
