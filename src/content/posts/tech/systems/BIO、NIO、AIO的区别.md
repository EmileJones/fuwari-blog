...
NIO 全称为 **Non-blocking I/O**（非阻塞输入/输出）。其非阻塞特性体现在：当通道（Channel）处于非阻塞模式时，调用 `read()` 或 `write()` 不会让线程阻塞，如果数据尚未准备好，方法会立即返回（Java中会返回0），而不是等待数据就绪。所以NIO需要通过**轮询**来保证数据被传输完成。
